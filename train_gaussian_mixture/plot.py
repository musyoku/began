...
def main():
	num_samples = 10000
	samples_true = sampler.gaussian_mixture_circle(num_samples, num_cluster=params["config"]["num_mixture"], scale=2, std=0.2)
	plot_scatter(samples_true, args.plot_dir, "scatter_true")
	plot_kde(samples_true, args.plot_dir, "kde_true")
	samples_fake = began.to_numpy(began.generate_x(num_samples, test=True))
